...
import { roles, roleLabel, userCan } from "@/lib/rbac";
    ...
    return (
      <>
        <PageHeader
          icon={Bot}
          title="Automações"
          subtitle="Faça login para gerenciar suas automações n8n"
        />
        <div className="rounded-xl border border-border bg-card p-8 text-center text-sm text-muted-foreground">
          Entre com sua conta do portal (mesma conta usada no n8n).
        </div>
      </>
    );
  }

  const canCreate = !!user && userCan(user, "automation.create");
  const isSuperior = !!user && userCan(user, "automation.admin");
  ...
  return (
    <>
      <PageHeader
        icon={Bot}
        title="Automações"
        subtitle={`n8n · login com seu e-mail do portal (${user.email}) · IP ${n8nUrl}`}
      />
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <span className="inline-flex items-center gap-1 rounded-md border border-border bg-card px-3 py-2 text-xs text-muted-foreground">
          <Shield className="size-3" /> Role: {roleLabel[role]}{" "}
          {isSuperior ? "· visão superior" : ""}
        </span>
      </div>
              ...
              <div className="mt-4 grid gap-2">
                {shares.filter((s) => s.ownerRole === r).length === 0 ? (
                  <p className="text-xs text-muted-foreground">
                    Nenhuma automação registrada para {roleLabel[r]}.
                  </p>
                ) : (
                  shares
                    .filter((s) => s.ownerRole === r)
                    .map((s) => (
                      <div
                        key={s.id}
                      ...
                      >
                        <div>
                          <p className="text-sm font-medium">
                            {s.workflowName}{" "}
                            <span className="text-xs text-muted-foreground">({s.workflowId})</span>
                          </p>
                          <p className="text-xs text-muted-foreground">
                            Dono: {s.ownerName} ·{" "}
                            {s.isPrivate
                              ? "Privado"
                              : s.sharedRole
                                ? `Compartilhado: ${roleLabel[s.sharedRole as Role]}`
                                : "Público (todas roles)"}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <a
                            href={`${n8nUrl}/workflow/${s.workflowId}`}
                            target="_blank"
                            rel="noopener noreferrer"
        ...
        <TabsContent value="geral" className="mt-4">
          <div className="rounded-xl border border-border bg-card p-4">
            <h3 className="flex items-center gap-2 text-sm font-semibold">
              <Share2 className="size-4" /> Automações compartilhadas
            </h3>
            <div className="mt-3 grid gap-2">
              {shares.filter((s) => !s.isPrivate).length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  Nenhuma automação compartilhada ainda.
                </p>
              ) : (
                shares
                  ...
                  .map((s) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {s.ownerName} ({roleLabel[s.ownerRole as Role]}) →{" "}
                          {s.sharedRole ? roleLabel[s.sharedRole as Role] : "todas"}
                        </p>
                      </div>
                      <a
                        href={`${n8nUrl}/workflow/${s.workflowId}`}
      ...
      {canCreate && (
        <div className="mt-6 rounded-xl border border-border bg-card p-4">
          <h3 className="flex items-center gap-2 text-sm font-semibold">
            <Bot className="size-4" /> Gerenciar workflows no n8n
          </h3>
          <p className="mt-1 text-xs text-muted-foreground">
            Crie e gerencie workflows diretamente pela API do n8n. Criar um workflow também o
            registra automaticamente para compartilhamento.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <input
              ...
              className="min-w-40 flex-1 rounded-md border border-input bg-card px-3 py-2 text-xs"
            />
            <button
            ...
            <div className="mt-3 grid gap-2">
              {workflows.map((wf) => (
                ...
                >
                  <div>
                    <p className="text-sm font-medium">
                      {wf.name} <span className="text-xs text-muted-foreground">(ID: {wf.id})</span>
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {wf.active ? "Ativo" : "Inativo"} · {wf.nodes.length} nó(s)
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <a
                    ...
                    {(isSuperior || String(wf.userId) === user?.id) && (
                      <button
                        onClick={() => deleteWfM.mutate(wf.id)}
                        disabled={deleteWfM.isPending}
          ...
          <h3 className="flex items-center gap-2 text-sm font-semibold">
            <Lock className="size-4" /> Registrar / compartilhar automação
          </h3>
          <p className="mt-1 text-xs text-muted-foreground">
            Após criar o workflow no n8n, registre-o aqui para gerir a visibilidade. Apenas você
            (dono) e roles superiores podem alterar. Privado = só você vê.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <select
              value={wfId}
          ...
          <p className="mt-2 flex items-center gap-1 text-[11px] text-muted-foreground">
            <Users className="size-3" /> Grupo: quem cria é dono; pode liberar para uma role
            específica ou para todos; privado fica só com o dono até liberar.
          </p>
        </div>
      )}
      ...
      <div className="mt-6 rounded-lg border border-warning/25 bg-warning-soft p-4 text-xs text-warning">
        ...
        <p className="mt-1">
          URL n8n: <code className="rounded bg-white px-1 py-0.5">{n8nUrl}</code> · API:{" "}
          <code className="rounded bg-white px-1 py-0.5">{n8nUrl}/api/v1</code>
        </p>
        <p className="mt-1">
          No Lovable: adicione um MCP Server do tipo HTTP/SSE com URL{" "}
          <code className="rounded bg-white px-1 py-0.5">{n8nUrl}/mcp</code> ou use Webhook URL{" "}
          <code className="rounded bg-white px-1 py-0.5">{n8nUrl}/webhook/...</code> gerada em cada
          workflow.
        </p>
        <p className="mt-1">
